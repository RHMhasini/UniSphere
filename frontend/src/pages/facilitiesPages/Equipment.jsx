            ...
            {filtered.map((resource) => (
              <article key={resource.id} className="eq-card">
                ...
                >
                  ...
                  <span className={`eq-card__status ${resource.status === 'ACTIVE' ? 'active' : 'oos'}`}>
                    {statusLabel(resource.status)}
                  </span>
                </div>
                <div className="eq-card__body">
                  <div>
                    <h2 className="eq-card__title">{resource.name || 'Untitled Equipment'}</h2>
                    <p className="eq-card__location">{resource.location || 'Location not specified'}</p>
                  </div>
