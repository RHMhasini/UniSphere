...
function TicketDetails() {
  ...
  const fetchTicketData = useCallback(async () => {
    try {
      ...
      try {
        ...
        console.error("Technician list fetch failed", e);
      }
  ...
  const handleDeleteTicket = async () => {
    if (role === "ADMIN") {
      if (
        !window.confirm(
          "Archive this ticket? It will be hidden from normal dashboards but kept in the database for audit.",
        )
      )
        return;
    } else {
      if (
        !window.confirm(
          "Remove this ticket from your list? Admins and technicians can still see and work on it.",
        )
      )
        return;
    }
  ...
  const submitCommentEdit = async (commentId) => {
    try {
      await ticketingApi.put(`/tickets/${id}/comments/${commentId}`, {
  ...
  const handlePostComment = async () => {
    ...
    setActionLoading(true);
    try {
      await ticketingApi.post(`/tickets/${id}/comments`, {
        message: newComment,
      });
      setNewComment("");
      fetchTicketData();
  ...
  const handleAssignTech = async (techEmail) => {
    try {
      await ticketingApi.put(`/tickets/${id}/assign`, {
  ...
  if (!isStaff && !isGeneral && !isCreator) {
    return (
      <div className="error-state">
        ...
        <h3>Access Denied</h3>
        <p>
          This is a specific issue raised by another user. You do not have
      ...
      <div className="detail-header-actions">
        ...
        <div className="actions-right">
          ...
          {isStaff && ticket.status === "IN_PROGRESS" && !resolutionPrompt && (
            <Button
              variant="success"
              onClick={() => setResolutionPrompt(true)}
              disabled={actionLoading}
            >
              Mark Resolved
            </Button>
          ...
          {role === "ADMIN" &&
            ...
            ticket.status !== "REJECTED" &&
            !rejectPrompt && (
              <Button variant="outline" onClick={() => setRejectPrompt(true)}>
                Reject Ticket
              </Button>
            )}
      ...
      {resolutionPrompt && (
        <div className="inline-prompt">
          ...
          <textarea
            ...
            placeholder="Explain the fix... (min 5 characters)"
            rows="3"
            maxLength={500}
          ></textarea>
          ...
          <div className="prompt-actions">
            <Button variant="ghost" onClick={() => setResolutionPrompt(false)}>
              Cancel
            </Button>
            <Button
              variant="primary"
              onClick={() => handleStatusChange("RESOLVED")}
              disabled={actionLoading || resolutionNote.trim().length < 5}
            >
              Complete Resolution
            </Button>
          </div>
      ...
      {rejectPrompt && (
        <div className="inline-prompt prompt-danger">
          ...
          <textarea
            ...
            placeholder="Reason for rejecting this request... (min 5 characters)"
            rows="3"
            maxLength={500}
          ></textarea>
          <div className="prompt-footer">
          ...
          <div className="prompt-actions">
            <Button variant="ghost" onClick={() => setRejectPrompt(false)}>
              Cancel
            </Button>
            <Button
              variant="primary"
            ...
            {ticket.attachments && ticket.attachments.length > 0 && (
              <div className="attachments-list">
                ...
                <p className="attachments-hint">
                  Thumbnails — click an image to view it full size.
                </p>
                <div className="file-grid">
                  {ticket.attachments.map((att, idx) => {
                    ...
                    const isUrl =
                      typeof att === "string" && /^https?:\/\//i.test(att);
          ...
          <div className="comments-section">
            ...
            <div className="comment-list">
              {comments.map((c) => (
                <div
                  key={c.id}
                  className={`comment-bubble ${c.userId === user?.email ? "mine" : ""}`}
                >
                  ...
                  {editingCommentId === c.id ? (
                    <div className="edit-mode">
                      ...
                      <div className="edit-actions">
                        ...
                        <button onClick={() => submitCommentEdit(c.id)}>
                          <Check size={14} />
                        </button>
                      </div>
                    </div>
            ...
            <div className="comment-composer">
              ...
              <div className="composer-footer">
                ...
                <Button
                  ...
                  disabled={
                    newComment.trim().length < COMMENT_MIN || actionLoading
                  }
                >
                  Post Comment
                </Button>
        ...
        <div className="detail-sidebar">
          <div className="sidebar-card">
            ...
            <div className="detail-row">
              ...
              >
                <User size={14} /> {getCreatorName(ticket.createdBy)}
              </strong>
            </div>
            <div
              className="detail-row"
              ...
              (ticket.status === "OPEN" || ticket.status === "IN_PROGRESS") ? (
                ...
                >
                  ...
                  {technicians.map((tech) => (
                    <option key={tech.id || tech.email} value={tech.email}>
                ...
                <strong>
                  {ticket.assignedTo
                    ? getCreatorName(ticket.assignedTo)
                    : "Unassigned"}
                </strong>
              )}
            </div>
          ...
          <div className="sidebar-card timeline-card">
            <h3>
              <Clock size={16} /> SLA Timeline & History
            </h3>
            <div className="timeline">
              ...
              {history.map((h, i) => {
                ...
                const prevTime =
                  i === 0 ? ticket.createdAt : history[i - 1].changedAt;
